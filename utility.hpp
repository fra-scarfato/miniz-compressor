...
static inline bool mapFile(const char fname[], size_t &size,
                           unsigned char *&ptr) {
  // open input file.
...
static inline bool allocateFile(const char fname[], const size_t size,
                                unsigned char *&ptr) {
  ...
  ptr = (unsigned char *)mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED,
                              fd, 0);
  if (ptr == MAP_FAILED) {
    if (QUITE_MODE >= 1)
...
static inline bool isDirectory(const std::filesystem::path &p,
                               size_t &filesize) {
  try {
...
// If compdecomp is true (compressing), it checks if fname has the suffix
// SUFFIX, if yes it returns true If compdecomp is false (decompressing), it
// checks if fname has the suffix SUFFIX, if yes it returns false
static inline bool discardIt(const char *fname, const bool compdecomp) {
  ...
  if (len > lensuffix &&
    ...
    return compdecomp; // true or false depends on we are compressing or
                       // decompressing;
  }
  return !compdecomp;
}
...
static inline char *getOption(char **begin, char **end,
...
static inline bool compressData(unsigned char *ptr, size_t size,
  ...
  if (compress(ptrOut, &cmp_len, (const unsigned char *)inPtr, inSize) !=
      Z_OK) {
    if (QUITE_MODE >= 1)
  ...
  // Write first the original file size (not really needed, but simplify
  // decompression)
...
// fname stores the file name of the file containing the data pointed by 'ptr'
// return true if okay, false in case of errors
static inline bool decompressData(unsigned char *ptr, size_t size,
                                  const std::string &fname) {
  size_t decompressedSize =
      reinterpret_cast<size_t *>(ptr)[0]; // read the original size
  ptr += sizeof(size_t);                  // advance the pointer

  // Write the decompressed data to a file
  unsigned char *decompressed_data = nullptr;
  std::string outfile =
      fname.substr(0, fname.size() - 4); // remove the SUFFIX (i.e., .zip)

  // allocate the space in a file for the uncompressed data. The file is memory
  // mapped.
...
static inline bool doWork(const char fname[], size_t size, const bool comp) {
  ...
  bool r = (comp) ? compressData(ptr, size, fname)
